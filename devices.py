...
class DeviceManager:
 ...
 workflow_id = workflow.get('id')
   ...
   Domoticz.Device(
    ...
    Used=1
        ...
        Domoticz.Log(f"Created device for workflow: {workflow_name} (Unit: {unit})")
 ...
 # Set initial state
    ...
    devices[unit].Update(nValue=1 if active else 0, sValue="On" if active else "Off")
  ...
  devices[unit].Update(nValue=new_state, sValue="On" if active else "Off")
         Domoticz.Debug(f"Updated workflow {workflow.get('name')}: {'Active' if active else 'Inactive'}")
